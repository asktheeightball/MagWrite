"""Display-only MagTag terminal for the live USB keyboard typing session.

Identical in role to the proven editor display harness: the MagTag never edits,
...
start, refresh completion, and the displayed revision.

Only three things differ from the editor display harness, and all three are
constructor arguments or config values rather than changes to proven code:

* a live run is operator-paced, so the run budget is far larger;
* the authorised ceilings for this phase are 100 viewports and 50 partial
  refreshes, so the completion history is sized above 51 entries;
* it has its own independent guard family.
"""
...
START = "/magwrite_usb_keyboard_display.started"
COMPLETE = "/magwrite_usb_keyboard_display.complete"
...
if not (
    ...
    and config.PHYSICAL_TEST_MODE == USB_KEYBOARD_DISPLAY_MODE
    and getattr(config, "USB_KEYBOARD_DISPLAY_TEST_MODE", "DISABLED")
        == USB_KEYBOARD_DISPLAY_MODE
):
    raise RuntimeError("MagTag USB keyboard display gate not armed")
...
if exists(START) or exists(COMPLETE):
    raise RuntimeError("MagTag USB keyboard display guard exists")
...
clock = RunClock(
    time.monotonic,
    getattr(config, "USB_KEYBOARD_ARMING_TIMEOUT_SECONDS", 1800),
    getattr(config, "USB_KEYBOARD_DISPLAY_TIMEOUT_SECONDS", 2700),
)
...
result = "FAIL"
reason = None
logger({"event": "usb_keyboard_display_ready",
...
try:
    while True:
        ...
        if not clock.running and scheduler.last_input_sequence:
            logger({"event": "usb_keyboard_run_clock_started",
        ...
        while len(outbox):
            ...
            bytes_sent += written
            logger({"event": "usb_keyboard_status_sent", "message_type": kind,
...
summary = {
    "event": "usb_keyboard_display_test_summary",
