...
def encode_status(message_type, fields):
    if message_type == STATUS_HELLO:
        flags = (1 if fields["receiver_ready"] else 0) | (2 if fields["display_ready"] else 0)
        return bytes((fields["protocol_version"], fields["app_version"])) + _u32(fields["displayed_revision"]) + bytes((flags,)) + _text(fields["test_id"], MAX_TEST_ID)
    if message_type == FRAME_ACCEPTED:
        return _u32(fields["received_sequence"]) + _u32(fields["pending_revision"]) + bytes((1 if fields["superseded"] else 0,))
    if message_type == REFRESH_STARTED:
        return _u32(fields["viewport_sequence"]) + bytes((fields["refresh_mode"],)) + _u32(fields["latest_received_revision"]) + _u32(fields["previous_displayed_revision"])
    if message_type == REFRESH_COMPLETED:
        return _u32(fields["viewport_sequence"]) + _u32(fields["duration_ms"]) + _u32(fields["latest_received_revision"]) + bytes((1 if fields["stale"] else 0,))
    if message_type == DISPLAY_CAUGHT_UP:
        return _u32(fields["displayed_revision"]) + _u32(fields["latest_received_revision"]) + _u32(fields["viewport_hash"])
    if message_type == FRAME_REJECTED:
        return _u32(fields["received_sequence"]) + _u32(fields["received_revision"]) + bytes((fields["code"],)) + _u32(fields["displayed_revision"]) + _text(fields["reason"], MAX_REASON)
...
def decode_status(message_type, payload):
    if message_type == STATUS_HELLO:
        ...
        flags = payload[6]
        return {"protocol_version": payload[0], "app_version": payload[1], "displayed_revision": int.from_bytes(payload[2:6], "big"), "receiver_ready": bool(flags & 1), "display_ready": bool(flags & 2), "test_id": _read_text(payload, 7, MAX_TEST_ID)}
    if message_type == FRAME_ACCEPTED:
        _require(payload, 9)
        return {"received_sequence": int.from_bytes(payload[0:4], "big"), "pending_revision": int.from_bytes(payload[4:8], "big"), "superseded": bool(payload[8])}
    if message_type == REFRESH_STARTED:
        _require(payload, 13)
        return {"viewport_sequence": int.from_bytes(payload[0:4], "big"), "refresh_mode": payload[4], "latest_received_revision": int.from_bytes(payload[5:9], "big"), "previous_displayed_revision": int.from_bytes(payload[9:13], "big")}
    if message_type == REFRESH_COMPLETED:
        _require(payload, 13)
        return {"viewport_sequence": int.from_bytes(payload[0:4], "big"), "duration_ms": int.from_bytes(payload[4:8], "big"), "latest_received_revision": int.from_bytes(payload[8:12], "big"), "stale": bool(payload[12])}
    if message_type == DISPLAY_CAUGHT_UP:
        _require(payload, 12)
        return {"displayed_revision": int.from_bytes(payload[0:4], "big"), "latest_received_revision": int.from_bytes(payload[4:8], "big"), "viewport_hash": int.from_bytes(payload[8:12], "big")}
    if message_type == FRAME_REJECTED:
        if len(payload) < 14:
            raise ValueError("malformed FRAME_REJECTED")
        return {"received_sequence": int.from_bytes(payload[0:4], "big"), "received_revision": int.from_bytes(payload[4:8], "big"), "code": payload[8], "displayed_revision": int.from_bytes(payload[9:13], "big"), "reason": _read_text(payload, 13, MAX_REASON)}
    if message_type == DISPLAY_ERROR:
        if len(payload) < 14:
            raise ValueError("malformed DISPLAY_ERROR")
        return {"code": payload[0], "inflight_revision": int.from_bytes(payload[1:5], "big"), "latest_received_revision": int.from_bytes(payload[5:9], "big"), "displayed_revision": int.from_bytes(payload[9:13], "big"), "reason": _read_text(payload, 13, MAX_REASON)}
    if message_type == TEST_COMPLETE:
        _require(payload, 18)
